                    ...
                    <div class="mb-3">
                        ...
                        <div>
                            <?php 
                                ...
                                error_log("Order status in view: " . (isset($order['status']) ? $order['status'] : 'NOT SET'));
                                
                                // Check if order status exists
                                if (!isset($order['status'])) {
                                    error_log("Order status is not set in the order array");
                                    $statusClass = 'secondary';
                                    $statusDisplay = 'Unknown';
                                } else {
                                    // Use helper function for status badge class
                                    $statusClass = getStatusBadgeClass($order['status']);
                                    $statusDisplay = ucfirst(htmlspecialchars($order['status']));
                                }
                            ?>
...
{
    error_log("getStatusBadgeClass called with status: " . ($status ?? 'NULL'));
    
    // Check if status is null or not a string
    if ($status === null) {
        error_log("getStatusBadgeClass received NULL status");
        return 'secondary';
    }
    
    if (!is_string($status)) {
        error_log("getStatusBadgeClass received non-string status: " . gettype($status));
        return 'secondary';
    }
    ...
    switch ($status) {
        ...
        case 'cancelled':
            return 'danger';
        default:
            error_log("getStatusBadgeClass received unknown status: {$status}");
            return 'secondary';
    }
}
?>